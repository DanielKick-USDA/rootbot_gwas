...
# if(TRUE){
  ...
  library("progress")
  pb <- progress::progress_bar$new(total = length(record_list))
...
```{r}
uid_code = 143
# There are values from the early data that aren't properly being processed:
# they are not receiving an uid 

# ── Variable type: numeric ─────────────────────────────────────────────────────────────────
#   skim_variable  n_missing complete_rate   mean     sd p0     p25     p50    p75 p100 hist 
# 1 length                 0         1       1.59   4.09  0   0.221   0.796   2.02 110. ▇▁▁▁▁
# 2 image_cassette         0         1      28.8   14.0   2  22      27      41     53  ▅▃▇▆▆
# 3 uid                  643         0.923 220.   125.    1 111     215     333    430  ▇▇▇▇▇

temp <- df[is.na(df$uid), ] %>% select(genotype, image_cassette, image) %>% distinct() # %>% summarise(image == sort(image)[1])
temp
...
for(uid_code in unique(metadata$uid)){
  ...
  # select first index because it's a tibble
  fuzzy_date     <- metadata[metadata$uid == uid_code, "rootbot_date"][[1]] 
...
data$hours_elapsed <- as.numeric((data$delta_datetime) / (60*60))
data 
```
